...
public class CompteEpargneDAO implements IDAO<Integer, CompteEpargne>{
    ...
    private static final String FIND_ID_QUERY = "SELECT * FROM compte WHERE id = ? AND type = 1";
    private static final String FIND_ALL_COMPTE_QUERY = "SELECT * FROM compte WHERE type = 1";
    ...
    public void create(CompteEpargne compteEpargne) throws SQLException, IOException, ClassNotFoundException {
        ...
        if (connection != null) {
            try (PreparedStatement ps = connection
                ...
                ps.setFloat(2, compteEpargne.getTauxInter());
                ps.setInt(3, compteEpargne.getType());
                ps.setInt(4, compteEpargne.getAgenceId());
                ps.executeUpdate();
            ...
            try (PreparedStatement ps = connection.prepareStatement(FIND_ALL_COMPTE_QUERY)) {
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        compteEpargne.setTauxInter(rs.getInt("taux_interet"));
                        compteEpargne.setType(rs.getInt("type"));
