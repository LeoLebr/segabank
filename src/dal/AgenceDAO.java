...
public class AgenceDAO implements IDAO<Integer, Agence> {
    ...
    private static final String INSERT_INTO_QUERY = "INSERT INTO agence (code, adresse) VALUES(?,?)";
    private static final String UPDATE_SET_QUERY = "UPDATE agence SET code = ?, adresse = ? WHERE id = ?";
    private static final String DELETE_QUERY = "DELETE FROM agence WHERE id = ?";
    private static final String FIND_ID_QUERY = "SELECT * FROM agence WHERE id = ?";
    private static final String FIND_ALL_QUERY = "SELECT * FROM agence";
    private static final String FIND_ALL_COMPTE_QUERY = "SELECT * FROM compte WHERE id_agence = ?";
    ...
    public void create(Agence agence) throws SQLException, IOException, ClassNotFoundException {
        ...
        if (connection != null) {
            try (PreparedStatement ps = connection
                    .prepareStatement(INSERT_INTO_QUERY, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void update(Agence agence) throws SQLException, IOException, ClassNotFoundException {
        ...
        Connection connection = PersistenceManager.getConnection();
        if (connection != null) {
            try (PreparedStatement ps = connection.prepareStatement(UPDATE_SET_QUERY)) {
    ...
    public Agence findById(Integer id) throws SQLException, IOException, ClassNotFoundException {
        ...
        Connection connection = PersistenceManager.getConnection();
        if (connection != null) {
            try (PreparedStatement ps = connection.prepareStatement(FIND_ID_QUERY)) {
                ps.setInt(1, id);
                    ...
                    while (rs.next()) {
                        switch (rs.getInt("type")) {
                            case 1 :
                                ...
                                compteEpagne.setTauxInter(rs.getInt("taux_inter"));
                            ...
                            case 2 :
                                ...
                                compteSimple.setUnDecouvert(rs.getFloat("unDecouvert"));
