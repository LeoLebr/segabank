...
public class CompteSimpleDAO extends Compte implements  IDAO<Integer, CompteSimple> {
    ...
    private static final String FIND_ID_QUERY = "SELECT * FROM compte WHERE id = ? AND type = 2";
    private static final String FIND_ALL_COMPTE_QUERY = "SELECT * FROM compte WHERE type = 2";
    ...
    public void create(CompteSimple compteSimple) throws SQLException, IOException, ClassNotFoundException {
        ...
        if (connection != null) {
            try (PreparedStatement ps = connection
                ...
                ps.setFloat(2, compteSimple.getUnDecouvert());
                ps.setInt(3, compteSimple.getType());
                ps.setInt(4, compteSimple.getAgenceId());
                ps.executeUpdate();
            ...
            try (PreparedStatement ps = connection.prepareStatement(FIND_ID_QUERY)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        compteSimple.setAgenceId(agence.getId());
                    }
                }
            }
        }
